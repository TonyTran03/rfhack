...
PairHackResult = namedtuple('result', ['df', 'auc', 'separation', 'iterations'])


class PairDatasetHacker:
    ...
    def hack(self, target_auc):
        ...
        for i in range(self.max_iter):
            mid = (lo + hi) / 2
            synth = make_blobs(self.stats, self.n_samples, mid)
            score = RFWrapper.from_pair(synth, self.real_df, self.target_col)
            if best is None or abs(score - target_auc) < abs(best.auc - target_auc):
                best = PairHackResult(synth, score, mid, i + 1)
